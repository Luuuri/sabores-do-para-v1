...
class TelaEstoque:
    ...
    def renderizar_linhas(self):
        ...
        for tipo, valor in self.filtros_ativos.items():
            if tipo == "categoria":
                ...
                filtrados = [
                    ...
                    if status_da_quantidade(i["quantidade"]) == valor
                ]
        ...
        for indice, item in enumerate(filtrados):
            ...
            ctk.CTkLabel(
                f,
                text=item.get("categoria", ""),
                font=ctk.CTkFont(size=13, weight="bold"),
                text_color="#1a1a1a"
            ).grid(row=linha, column=1, sticky="w", padx=8, pady=10)
            ...
            ctk.CTkLabel(
                ...
                text_color="#555"
            ).grid(row=linha, column=3, sticky="w", padx=8, pady=10)

            BadgeStatus(
                f,
                status_da_quantidade(item["quantidade"]),
                self.cores
            ).grid(row=linha, column=4, sticky="w", padx=8, pady=10)
            ...
            Botoes.BotaoEditar(
                ...
                self.cores,
                self.icones,
                command=lambda i=item: self.abrir_editar_item(i)
            ).pack(side="left")
            ...
            Botoes.BotaoApagar(
                ...
                self.cores,
                self.icones,
                command=lambda i=item: self.ao_excluir(i)
            ).pack(side="left")
    ...
    def construir_formulario(self, item: dict = None):
        ...
        if item:
            ...
            entrada_custo_produto.insert(0, str(item.get("custo_produto", "")))
    ...
    def ao_excluir(self, item: dict):
        if excluir_item(item["id_estoque"], item["nome"]):
            self._itens = [
                i for i in self._itens
                if i["id_estoque"] != item["id_estoque"]
            ]
            self.renderizar_linhas()